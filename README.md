...
runs on Ambient. The bridge translates the Anthropic Messages API to Ambient's OpenAI-compatible
endpoint and, in one place, maps tool-call ids (reversibly), synthesizes streaming,
renders a context overflow as the `prompt is too long` signal Claude Code compacts
...
it spins up when demand arrives, and the CLI names serving
