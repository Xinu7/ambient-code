...
Messages API to Claude Code and translates every turn to Ambient's OpenAI-compatible
`/v1/chat/completions` endpoint. In one place it:

- **maps tool-call ids** (reversibly): OpenAI-style ids like `functions.Read:0` use a
  different grammar than Anthropic's `tool_use.id`; the bridge maps them so Claude Code
  only ever sees Anthropic-form ids that round-trip cleanly across turns.
- **synthesizes streaming**: it streams the OpenAI-compatible path and builds the
  Anthropic SSE locally, with an early `message_start` + keepalive pings so a long
  reasoning turn never idle-times-out.
- **renders a context overflow as the exact `prompt is too long` error**
